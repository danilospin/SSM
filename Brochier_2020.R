...
nd=ns=n=beta=betahat=e=ge=w=omegaw=omegaf=omega=phat=gu=Fn=What=p=phat=gu=k=yd=te=yfc=id=gd=c=UC=vector()
...
for(t in 2:100) 
  ...
  y[t]=(alpha2*vh[t-1]+(y[t-1]*sigma))*(1-alpha1*(1-tau)*(1/beta[t-1]*(1+betahat[t-1]))*(W[t-1]*(1+What[t-1]))/p[t]-(h[t-1]+h[t-1]*gamma*(u[t-1]-un)))^(-1)
  Y[t]=y[t]*p[t]
